...
- 最终可复制区块仍放在带围栏的 `text` 或 `plain` 代码块中，作为 Markdown 源记录和机器可读追溯；HTML 不再在页面底部渲染 `基本面速记` 或完整 `交易决策摘要`。
- `交易决策摘要` 仍置于源记录的基本面备注之后。顶部 `结论速览` 依次展示评级标签、一句话结论、当前位置与交易决策；不再展示核心理由、主要风险 / 关注双栏。对应的正文分析和折叠尾注继续保留。
- 顶部交易决策采用一行动作与仓位摘要、执行/观察条件和失效/重审条件两栏，窄屏上下排列。只突出动作、关键数字与关键词，避免整段加粗。顶部不展示有效期，正文和 Markdown 源记录仍保留 `建议有效期`。
- `查看仓位与止损完整说明` 默认折叠；仅当明确 `持仓状态：未持仓` 且 `当前动作：等待` 时，隐藏整个折叠区及入口。已持仓、状态未知/缺失或其他动作均保留。不得将建议仓位 0% 推断为未持仓；即使隐藏完整说明，观察条件和失效/重审条件仍展示。
...
为生成顶部交易决策卡，第 3.3 节必须各自使用以下单行标签：`持仓状态`（严格为 `未持仓 / 已持仓 / 未知` 中的一项）、`当前动作`、`建议目标仓位`、`参考价格与计划买入区间`（无买入计划时写观察或不追高条件）、`价格止损`、`基本面提前退出条件`、`建议有效期`。持仓状态以用户明确提供的信息为准，未提供时写 `未知`，并与末尾源记录保持一致。这些字段是正文事实；完整的估值推导、盈亏比、置信度和反证仍留在本节正文。

新报告同时提供以下单行摘要字段，供顶部版面提取：`仓位速览`（实际持仓状态与建议仓位，未知时分别给出未持仓/已持仓条件）、`观察条件速览`（参考价格及日期、入场或观察条件）、`止损速览`（止损价格及必要限制；无计划或依据不足时说明）。摘要必须忠于对应完整字段，保留关键前提，不新增事实；可用中文分号拆分独立观察项，用 Markdown 加粗关键数字。`基本面提前退出条件` 完整显示，保留“且/或”等逻辑关系。兼容旧报告时，缺少摘要字段可回退到原完整字段，不机械截断。
