...
- 新生成或后续更新的报告采用“先图后表”：渲染器从标准财务表生成静态 SVG 图表，并将第 1、2 节的原始财务明细默认折叠；不得改写或批量重新渲染历史报告以套用新样式。
...
- 顶部 `结论速览` 仅展示四项评级和一句话结论；投资价值与介入条件只在正文 3.3 展示，估值计算放第 8 节。
...
顶部图表区域统一命名为 `财务趋势`，只展示以上两张趋势图，宽屏两列并排、窄屏上下排列。不再展示业务收入结构图及其来源/未生成原因占位；第 2.2 节分业务表格与业务结构判断继续保留。
