...
- `交易决策摘要` 仍置于源记录的基本面备注之后。渲染器从第 3.3 节提取并在顶部结论卡压缩展示：当前动作、建议仓位、执行/观察条件、失效条件和有效期；其余推理仅保留在正文第 3.3 节。
...
为生成顶部交易决策卡，第 3.3 节必须各自使用以下单行标签：`当前动作`、`建议目标仓位`、`参考价格与计划买入区间`（无买入计划时写观察或不追高条件）、`价格止损`、`基本面提前退出条件`、`建议有效期`。这些字段是正文事实；完整的估值推导、盈亏比、置信度和反证仍留在本节正文。
