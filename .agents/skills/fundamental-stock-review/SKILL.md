...
- 每张图表必须标注数据期间、统计口径和信息边界中的主要数据来源；SVG 仅用于帮助阅读，表格和来源仍是核对基准。
...
- `交易决策摘要` 仍置于源记录的基本面备注之后。顶部 `结论速览` 依次展示评级标签、一句话结论、当前位置与交易决策；不再展示核心理由、主要风险 / 关注双栏及底部口径说明。对应的正文分析和信息边界继续保留。
